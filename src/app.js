...
import { loginUI, initLogin } from "./login.js"; // UI comes from script
...
(function (global) {
  ...
  // CAPTURE THE UI: Do this at the very top.
  // This saves your "App Registration" HTML into memory.
  const registrationUI = container.innerHTML;

  function render() {
    const isLoggedIn = checkSession();

    if (!isLoggedIn) {
      // If not logged in, clear the registration UI and show login
      ...
      // If logged in, restore the saved HTML from memory
      container.innerHTML = registrationUI;

      // IMPORTANT: Because innerHTML wipes event listeners,
      // you must re-bind your buttons here.
      startApp();
    }
  }
      ...
      if (isUpdate) {
        if (appNameInput.value) {
          const appExists = appList.some(
            (app) =>
              app.name === appNameInput.value && app.id !== selectedApp.id
          );
